...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if a new event image file is provided
    if (!empty($_FILES['newEventImage']['name'])) {
        // Create a new file name based on the updated event title
        $newFileName = strtolower($newEventTitle) . '.png';
        $uploadFile = $uploadDir . $newFileName;

        if (move_uploaded_file($_FILES['newEventImage']['tmp_name'], $uploadFile)) {
            // File uploaded successfully
            echo "File is valid and was successfully uploaded.";

            // Update the event image path in the database
            $updateImagePathSql = "UPDATE events SET event_image_path = '$uploadFile' WHERE id = $event_id";
            $conn->query($updateImagePathSql);
        } else {
            echo "Error uploading the new event image.";
        }
    }
    $newEventTitle = $_POST['newEventTitle'];
...
<body>
    ...
    <div class="page-container">
        ...
        <form action="" method="post">
            ...
            <label for="newEventImagePath">New Event Image Path</label>
            <input type="file" id="newEventImagePath" name="newEventImagePath"> 
